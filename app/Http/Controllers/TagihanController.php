...
class TagihanController extends Controller
{
    ...
    public function bukaTagihanApi(Request $request)
    {
        $request->validate([
        ...
        Log::info('Filtering for bulan: ' . $bulan . ', tahun: ' . $tahun);
        ...
        Log::info('Query Result:', ['pelangganBelumBayar' => $pelangganBelumBayar->toArray()]);

        if ($pelangganBelumBayar->isEmpty()) {
            return response()->json(['message' => 'No data found'], 404);
        }

        return response()->json($pelangganBelumBayar);
    }



    // public function dataTagihanApi(Request $request)
    // {
    //     $request->validate([
    //         'bulan' => 'required',
    //         'tahun' => 'required',
    ...
    //     $tahun = $request->input('tahun');
    //     $tagihanList = Tagihan::getDataByMonthYearAndStatus($bulan, $tahun, 'BL');
    //     return response()->json($tagihanList);
    // }
    ...
    public function dataTagihanApi(Request $request)
    {
        ...
        logger()->info('Query Result:', ['tagihanList' => $tagihanList->toArray()]);
    ...
    public function bayarTagihanApi($kode)
    {
        $tagihan = Tagihan::find($kode);
        if (!$tagihan) {
        ...
        return response()->json(['message' => 'Pembayaran tagihan berhasil'], 200);
    }


    public function lunasTagihanApi()
    {
        $pelangganLunas = Pelanggan::where('status', 'aktif')
        ...
        return response()->json($pelangganLunas);
    }


    public function belumLunasApi(Request $request)
    {
        $request->validate([
            'bulan' => 'required',
            'tahun' => 'required|integer',
        ...
        logger()->info('Query Result:', ['pelangganBelumLunas' => $pelangganBelumLunas->toArray()]);

        if ($pelangganBelumLunas->isEmpty()) {
            return response()->json(['message' => 'No data found'], 404);
        }

        return response()->json($pelangganBelumLunas);
    }


    public function deleteTagihanApi($id)
    {
        $tagihan = Tagihan::find($id);
        if (!$tagihan) {
            return response()->json(['message' => 'Tagihan tidak ditemukan'], 404);
