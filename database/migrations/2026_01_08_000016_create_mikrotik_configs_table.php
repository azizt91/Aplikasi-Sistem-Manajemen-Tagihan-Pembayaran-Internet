...
/**
 * Migration untuk membuat tabel mikrotik_configs.
 * Menyimpan konfigurasi koneksi ke perangkat MikroTik.
 */
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('mikrotik_configs', function (Blueprint $table) {
            $table->id();
            $table->string('name', 255)->comment('Nama konfigurasi MikroTik');
            $table->string('ip_address', 255)->comment('IP Address MikroTik');
            $table->integer('port')->default(8728)->comment('Port API MikroTik');
            $table->string('username', 255)->comment('Username MikroTik');
            ...
            $table->enum('connection_status', ['connected', 'failed', 'error', 'never_tested', 'disconnected'])
